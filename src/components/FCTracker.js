...
export default function FCTracker() {
    ...
    if (tournament && knockoutRound !== null && knockoutMatchIdx !== null) {
      ...
      if (winner) {
        ...
        if (!updatedTournament.knockoutBracket[nextRound]) {
          updatedTournament.knockoutBracket[nextRound] = new Array(Math.ceil(updatedTournament.knockoutBracket[knockoutRound].length / 2)).fill(null)
        }
  ...
  const performDraw = async () => {
    ...
    if (tournamentType === 'knockout') {
      ...
      flattened.forEach((pid, idx) => { if (idx < bracketSize) bracket[idx] = pid })
  ...
  const captureTwitch = async () => {
    ...
    } catch { setOcrResult({ error: 'Could not capture stream. Is it live?' }); setOcrProcessing(false) }
  }
  ...
  const processOCR = async (base64, mediaType) => {
    ...
    } catch { setOcrResult({ error: 'Failed to process image' }) }
    ...
    <div style={s.container}>
      ...
      {showKnockoutMatch && <Modal onClose={() => setShowKnockoutMatch(null)} title="KNOCKOUT MATCH">
        ...
        <div style={s.modalBtns}>
          ...
          <button onClick={() => addMatch(showKnockoutMatch.p1, showKnockoutMatch.p2, matchData.score1, matchData.score2, showKnockoutMatch.round, showKnockoutMatch.matchIdx)} style={s.btnAcc}>Submit</button>
        </div>
      </Modal>}
      ...
      {showTeamSelect && <Modal onClose={() => setShowTeamSelect(false)} title="SELECT TEAMS" wide>
        <p style={s.modalDesc}>Each player picks their team</p>
        ...
        <div style={s.modalBtns}><button onClick={() => setShowTeamSelect(false)} style={s.btnSec}>Back</button><button onClick={() => { setShowTeamSelect(false); setShowDraw(true); performDraw() }} style={s.btnAcc}>Draw</button></div>
      </Modal>}
      ...
      {showOCR && <Modal onClose={() => { setShowOCR(false); setOcrResult(null) }} title="SCAN SCORE" wide>
        <div style={s.ocrTabs}>
          <div style={s.ocrTab}>
            ...
            <button onClick={captureTwitch} disabled={ocrProcessing} style={s.btnPri}>Capture</button>
          </div>
        ...
        {ocrResult && !ocrResult.error && (
          <div style={s.ocrResult}>
            ...
            <button onClick={() => addMatch(matchData.player1, matchData.player2, ocrResult.score1, ocrResult.score2)} disabled={!matchData.player1 || !matchData.player2} style={s.btnAcc}>Confirm</button>
          </div>
        )}
...
const TeamSearchSelect = ({ value, onChange }) => {
  ...
  return (
    <div ref={ref} style={s.teamSelect}>
      <div style={s.teamSelectTrigger} onClick={() => setOpen(!open)}>
        {value ? <div style={s.teamSelectValue}><span style={s.teamInitials}>{value.slice(0, 2).toUpperCase()}</span><span>{value}</span></div> : <span style={s.teamSelectPlaceholder}>Select team...</span>}
      </div>
      {open && (
        <div style={s.teamDropdown}>
          <input type="text" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)} style={s.teamSearchInput} autoFocus />
          <div style={s.teamList}>
            {filtered.map((t, i) => (
              <div key={i} style={s.teamOption} onClick={() => { onChange(t.name); setOpen(false); setSearch('') }}>
                ...
                <div style={s.teamOptionInfo}><span style={s.teamOptionName}>{t.name}</span><span style={s.teamOptionLeague}>{t.league}</span></div>
              </div>
            ))}
          </div>
        </div>
