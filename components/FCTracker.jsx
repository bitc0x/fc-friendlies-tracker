...
const FCTracker = () => {
  ...
  useEffect(() => {
    const loadData = () => {
      try {
        const keys = ['fct-players', 'fct-matches', 'fct-tournament', 'fct-seasons', 'fct-current-season', 'fct-twitch'];
        const results = keys.map(k => storage.get(k));
        if (results[0]?.value) setPlayers(JSON.parse(results[0].value));
        if (results[1]?.value) setMatches(JSON.parse(results[1].value));
        if (results[2]?.value) setTournament(JSON.parse(results[2].value));
        if (results[3]?.value) setSeasons(JSON.parse(results[3].value));
        if (results[4]?.value) setCurrentSeason(JSON.parse(results[4].value));
        if (results[5]?.value) setTwitchChannel(JSON.parse(results[5].value));
  ...
  const save = (key, data, setter) => { setter(data); storage.set(key, JSON.stringify(data)); };
  ...
  const addMatch = (p1, p2, s1, s2, knockoutRound = null, knockoutMatchIdx = null) => {
    ...
    if (tournament && knockoutRound !== null && knockoutMatchIdx !== null) {
      ...
      if (winner) {
        ...
        if (!updatedTournament.knockoutBracket) updatedTournament.knockoutBracket = {};
        if (!updatedTournament.knockoutBracket[nextRound]) {
          updatedTournament.knockoutBracket[nextRound] = new Array(Math.ceil(updatedTournament.knockoutBracket[knockoutRound]?.length / 2 || 4)).fill(null);
        }
        
        if (!updatedTournament.knockoutBracket[nextRound]) updatedTournament.knockoutBracket[nextRound] = [];
        while (updatedTournament.knockoutBracket[nextRound].length <= nextMatchIdx * 2 + 1) {
          updatedTournament.knockoutBracket[nextRound].push(null);
        }
